...
import { select as d3Select, event as d3Event } from 'https://cdn.jsdelivr.net/npm/d3-selection@3/+esm';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  d3Select(svgEl)
    .call(d3Zoom().scaleExtent([CONFIG.ZOOM_MIN, CONFIG.ZOOM_MAX])
      .on('zoom', () => {
        svgEl.style.transform = d3Event.transform.toString();
      })
    );
  ...
  yearFilter.addEventListener('click', e => {
    ...
    yearFilter.querySelectorAll('.year-btn').forEach(b=>b.classList.remove('text-white'));
